...
| Add repair/backfill commands for legacy batches | P2 | done | Added `repair.py audit` for legacy metadata, missing audio/tokens, and phrase-like sentence input drills; added `repair.py audio` for targeted audio backfill; added validation-backed `repair.py tokens` for word-only sentence token backfill. |
...
| Plan local transcription workflow | P2 | done | Added `TRANSCRIPTION_PLAN.md` with local Whisper backend options, data surfaces, CLI shape, reference-text correction, standalone transcript export, transcript-linked sentence-card enrichment, viewer integration, and phased implementation. |
