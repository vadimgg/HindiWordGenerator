...
- make role transitions visible in user-facing updates when the work changes
  mode, such as `Acting as project-manager`, `Acting as python-engineer`, or
  `Acting as python-reviewer`
- route non-trivial specialist work to the focused agent instead of retaining
  ...
  python-reviewer for runtime architecture/data drift, doc-writer for docs,
