...
```text
# Source title
## Chapter or topic subtitle
हिन्दी वाक्य (romanisation);English translation
```

The first non-empty line must be a non-empty `# Source title` heading. Prefer a
second `## Chapter or topic subtitle` line for textbook/course material. Do not
accept filename-derived labels as sufficient for input QA.
...
- Missing structural guard in scripts: recommend a `process.py` validator task
  for `agents/packs/schema-guardian/AGENT.md`.
