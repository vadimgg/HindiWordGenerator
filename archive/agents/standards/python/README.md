...
Python and CLI standards for HindiWordGenerator, adapted from the reference
agent standards without requiring Brief.
...
| Rule | When to read |
|---|---|
| [Architecture Boundaries](rules/architecture-boundaries.md) | Changing module ownership, imports, pipeline flow, or data surfaces. |
| [File And Function Size](rules/file-function-size.md) | Touching large files/functions or planning refactors. |
| [Single Responsibility](rules/single-responsibility.md) | Splitting or adding functions/modules. |
| [Reuse Before Helpers](rules/reuse-before-helpers.md) | Adding parser, path, schema, batch, display, or audio helpers. |
| [Constants And Magic Values](rules/constants-and-magic-values.md) | Adding repeated strings, keys, defaults, limits, or labels. |
| [Names](rules/names.md) | Naming new modules, functions, flags, or helpers. |
| [Error Handling](rules/error-handling.md) | Changing user-facing failures or validation errors. |
| [CLI Output](rules/cli-output.md) | Changing command output, summaries, warnings, or help. |
| [Testing](rules/testing.md) | Changing behavior or validation. |
| [Schema And Parsing](rules/schema-and-parsing.md) | Reading CSV/JSON, validating schema, writing output, or handling transcripts. |
| [Data Surface Drift](rules/data-surface-drift.md) | Touching input, output, audio, manifest, prompts, or viewer data. |
