...
--------
check
    Scan input/words/ and input/sentences/ YAML files and print a JSON array of
...
from batch_planner import (
    ...
    batch_file_path,
    build_batch_yaml,
    build_batch_yaml_from_metadata,
...
def cmd_mark_done(pipeline_type: str, stem: str, batches: int, count: int):
    ...
    input_dir   = cfg["input"]
    source_path = input_dir / f"{stem}.yaml"

    if not source_path.exists():
        print(f"Error: {source_path} not found", file=sys.stderr)
        sys.exit(1)

    record_processed(pipeline_type, stem, source_path, prompt_file, batches, count)
...
def main():
    ...
    p_write.add_argument("stem",         help="YAML filename without extension, e.g. hindi_01")
    ...
    p_done.add_argument("stem",    help="YAML filename without extension, e.g. hindi_01")
