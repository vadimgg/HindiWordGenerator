...
def source_from_yaml_item(item: dict) -> dict:
    hindi = str(item.get("hindi", "")).strip()
    if not hindi:
        raise ValueError("Could not parse Hindi from YAML item.")
    return {
        "hindi": hindi,
        "source_romanisation": str(item.get("romanisation", "")).strip(),
        "source_english": str(item.get("english", "")).strip(),
    }
