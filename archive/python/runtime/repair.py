...
from batch_planner import parse_yaml_metadata
...
def _looks_like_sentence_drill(item: dict) -> bool:
...
def _audit_sentence_input(path: Path) -> list[dict]:
    _, items = parse_yaml_metadata(path)
    issues = []
    for index, item in enumerate(items, 1):
        if _looks_like_sentence_drill(item):
            issues.append({
                "path": str(path),
                "kind": "sentence-input-phrase-drill",
                "detail": f"item {index} looks phrase-like: {item.get('hindi', '')}",
            })
    return issues
...
def audit(pipeline_type: str | None, include_inputs: bool) -> list[dict]:
    ...
    if include_inputs and pipeline_type in (None, "sentences"):
        for path in sorted(PIPELINES["sentences"]["input"].glob("*.yaml")):
