...
Use this agent to review command names, help text, arguments, interactive
...
- check whether each flag name matches its actual blast radius; users should not
  assume a flag limits more files, state, network calls, or side effects than it
  really does
...
- verify users can quickly find the relevant id, file, result, warning, and
  next command in normal and error output
...
- push back on flags whose names imply a narrower scope than the command
  actually uses; help/output must state what is checked, prepared, written, or
  left untouched

## Command Naming Rules

- Prefer intent-level names such as `change new`, `task done`, `review ready`,
  or `followup archive`.
...
- Prefer subcommands when the action belongs to a domain, such as
  `followup list`.
...
- Do not make users infer whether a target/filter flag limits all work or only
  one phase such as adapter writes, display, validation, or reporting.
...
Example:
  tool bug "Archive writes the wrong timestamp"
```
...
- cyan: ids and refs such as `WP01`, `BL003`, spec refs, PR numbers
