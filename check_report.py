...
class StemPlan:
    ...
    missing_metadata_heading: bool = False
...
def build_stem_plans(
    ...
    for current_type in types:
        ...
        for csv_path in sorted(input_dir.glob("*.csv")):
            display_label, items = process.parse_csv(csv_path)
            if not items:
                continue
            ...
            stem_plan = StemPlan(
                ...
                stem=csv_path.stem,
                display_label=display_label or process.label_from_stem(csv_path.stem),
                existing_batches=existing_batches,
                missing_metadata_heading=not process.has_top_metadata_heading(csv_path),
            )
            ...
            for offset, batch in enumerate(batches, 1):
                job = generate.BatchJob(
                    ...
                    csv=process.build_batch_csv(stem_plan.display_label, batch),
                    count=len(batch),
                )
                all_jobs.append(job)
...
def render_check(
    ...
    for stem_plan in stem_plans:
        if not (
            ...
            or stem_plan.missing_metadata_heading
        ):
            continue
        gaps: list[str] = []
        if stem_plan.missing_metadata_heading:
            gaps.append("metadata_heading")
    ...
    for stem_plan in stem_plans:
        if not any([
            ...
            stem_plan.missing_metadata_heading,
        ]):
            continue
        ...
        if stem_plan.missing_metadata_heading:
            console.print(
                "[magenta]Missing source/topic heading:[/magenta] "
